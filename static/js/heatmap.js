...
class HeatmapPlugin {
  ...
  constructor(
    ...
    this._options.range.from = this._options.range.from.replace(
      /#year#/i,
      new Date().getFullYear()
    );
    this._options.range.till = this._options.range.till.replace(
      /#year#/i,
      new Date().getFullYear()
    );
  ...
  render() {
    ...
    while (current <= target) {
      ...
      html +=
        ...
        "px; " +
        '" class="entry color-';
      if ("undefined" !== typeof this._data[unit])
        html +=
          this._data[unit] +
          '" title="' +
    ...
    this._container.style.height =
      this._theme.height * this._options.range.rows +
      2 * this._options.range.rows +
      "px";
    this._container.innerHTML = html;
    if (this._options.legend) this.addLegend();
  }
  addLegend() {
    let legend = document.createElement("div", {});
    legend.className = "heatmap-legend text-black";
    let html = "less ";
    for (let option = 0; option < this._options.units; option++) {
      html +=
        '<div style="' +
        "height: " +
        this._theme.height / 2 +
        "px; " +
        "width: " +
        this._theme.width / 2 +
        "px; " +
